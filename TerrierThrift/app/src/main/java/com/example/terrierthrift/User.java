...
public class User{
    ...
    public List<ItemListing> getListings() { return listings; }
    public boolean getVerification(){ return isVerifiedUser;}
...
public static void submitReport(String targetType, int targetID, String reason, String details) {
    ...
    +createListing(details: Map, photos: List<Photo>): ItemListing
    +getListings(): List<ItemListing>
    +startSearch(criteria: FilterCriteria): SearchEvent
    +owns(listing: ItemListing): boolean
    +choosePaymentMethod(methodType: string)
    +createTransaction()
    +submitReport(targetType: string, targetID: int, reason: string, details: string): void
    }
